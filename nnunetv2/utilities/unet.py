...
class SAMConvUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # (n, c:384, 8, 8, 8) (skips[4].shape[2], skips[4].shape[3], skips[4].shape[4])=(6,10,10)
        sam_embed = F.interpolate(sam_embed, size=(skips[4].shape[2], skips[4].shape[3], skips[4].shape[4]), mode='trilinear', align_corners=True)

        skips[4] = torch.cat((skips[4], sam_embed), dim=1)
        # for s in skips:
        #     print(s.shape)
